...
totem {
    ...
    interface {
        ringnumber: 0
        bindnetaddr: 192.168.51.0  # Internal network
        mcastport: 5405
    }
}
nodelist {
    node {
        ring0_addr: haproxy1-internal
        nodeid: 1
    }
    node {
        ring0_addr: haproxy2-internal
        nodeid: 2
    }
}
quorum {
...
if [[ "$(hostname)" == "haproxy1" ]]; then
    ...
    # Authenticate with both nodes using pcs host auth
    echo "Authenticating with haproxy1-internal and haproxy2-internal..."
    sudo pcs host auth haproxy1-internal haproxy2-internal -u hacluster -p hacluster 

    # Create and start the cluster
    echo "Creating and starting the cluster..."
    sudo pcs cluster setup --name ProxyCluster haproxy1-internal haproxy2-internal
    ...
    sudo pcs constraint location g_proxy prefers haproxy1-internal=100
    sudo pcs constraint location g_proxy prefers haproxy2-internal=50
